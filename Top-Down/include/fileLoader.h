...
public:
    ...
    readFile(std::string base):readFileClock_("ReadFile"){
        ...
        m_upperBound = base+"graph.upperBound";
        m_sortUB = base+"graph.sortUpperBound"; 
        ...
        len = 0, fd = -1;
        addr = NULL;
        memEdges = 50000000;   //the number of edges loaded in dram
        m_maxID = 100000000;   //max 100M vertex
        write_io = 0;
    ...
    void mergeByDegSort(int size, uint32_t &vertexNum, uint32_t &maxDegree, string &name, bool flag);
