...
private:
	...
	ui *count; //record the number of key
	...
	std::ofstream tmpWrite;
    

public:

	ListLinearHeapTruss(ui _n, ui _key_cap) {
		n = _n;
		key_cap = _key_cap;
		...
		heads = keys = count = pres = nexts = nullptr;
		tmpWrite.open("result/record.txt");
	}
	~ListLinearHeapTruss() {
		if (heads != nullptr) {
			delete[] heads;
	...
	void init(ui _n, ui _key_cap, ui *_keys) {
		...
		if (count == nullptr) count = new ui[key_cap + 1]();
		...
		for (ui i = 0; i < _n; i++) insert(i, _keys[i]);
	}
	...
	void init(ui _n, ui _key_cap, ui *_ids, ui *_keys, ui *leader) {
		if (keys == nullptr) keys = new ui[n];
		if (pres == nullptr) pres = new ui[n];
		if (nexts == nullptr) nexts = new ui[n];
		if (heads == nullptr) heads = new ui[key_cap + 1];
